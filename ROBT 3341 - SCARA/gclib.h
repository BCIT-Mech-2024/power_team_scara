...
* \ingroup c
* \brief Discover available controllers and open connections
* \defgroup controller Controller
* \ingroup c
* \brief Manage a Galil controller
* \defgroup communication Communication
* \ingroup controller
* \brief Send commands
* \defgroup memory Memory
* \ingroup controller
* \brief Manage controller memory, such as program and arrays
* \defgroup unsolicited Unsolicited Data
* \ingroup controller
* \brief Receive messages, interrupts, and data records
* \defgroup remote Galil Connect
* \ingroup c
* \brief Host or connect to a remote gcaps instance
*/
...
#ifdef _MSC_VER
#define GCLIB_DEPRECATED __declspec(deprecated)
#elif defined(__GNUC__) | defined(__clang__)
#define GCLIB_DEPRECATED __attribute__((__deprecated__))
#else
...
#define G_PUBLISH_SERVER 1 //!< For GPublishServer(), use this value to publish server to local network.
#define G_REMOVE_SERVER 0 //!< For GPublishServer(), use this value to remove server from local network.
...
#define G_UTIL_ASSIGN 133 //!< GUtility(), assign IP addresses over the network.
...
#define G_UTIL_GCAPS_KEEPALIVE 258 //!< GUtility(), Deprecated 20210119. No longer functional.
...
#define G_UTIL_GCAPS_ASSIGN 261 //!< GUtility(), assign IP addresses over the network from the @ref gcaps server.
...
#define G_UTIL_GCAPS_LIST_SERVERS 263 //!< GUtility(), get a list of all available gcaps servers on the local network.
#define G_UTIL_GCAPS_PUBLISH_SERVER 264 //!< GUtility(), make local gcaps server discoverable by other gcaps servers on the local network.
#define G_UTIL_GCAPS_SET_SERVER 265 //!< GUtility(), set the new active gcaps server.
#define G_UTIL_GCAPS_SERVER_STATUS 266 //!< GUtility(), get information on the local server's name and if it is published to the local network.
#define G_UTIL_GCAPS_REMOTE_CONNECTIONS 267 //!< GUtility(), get a list of remote addresses connected to local server.
#define G_UTIL_GCAPS_SERVER_INFO 268
#define G_UTIL_GCAPS_ADDRESSES_GET_REMEMBERED 269 //!< GUtility(), returns true if gcaps is remembering ip assignments
#define G_UTIL_GCAPS_ADDRESSES_SET_REMEMBERED 270 //!< GUtility(), sets if gcaps should remember ip assignments
...
#define G_LINE_BUFFER 80 //!< For writes, via command interpreter, to the Galil.
	...
	*  \deprecated This function will be removed in a future gclib version. [Contact Galil](https://www.galil.com/contact) for needs not met by the other gclib functions.
	*  \param g Connection's handle.
	...
	*  The data will be null terminated unless the function returns `G_BAD_LOST_DATA` due to the buffer being too small to hold the data.
  ...
  *
    ...
    *  If multiple messages have been sent from the controller since the last call to `GMessage()`, they will all be placed in the buffer, separated by newline characters.
	*
	*  \param g Connection's handle.
	...
	*
	*    * @ref G_UTIL_ERROR_CONTEXT More error detail for the last error on @ref GCon, where available.
	*      The internal error message is cleared upon read.
	*      * `memory1` is output and must be a `char*`. Data will be null terminated, even if the data must be truncated to do so.
	*      * `memory2` is input and must be an `unsigned int*` holding the length of the buffer in `memory1`.
	*
